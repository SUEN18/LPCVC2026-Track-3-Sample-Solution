...
This repository contains the sample solution for LPCVC 2026 Track 3:AI Generated Images Detection. It also contains instructions on how to prepare the necessary files needed for submission.
...
Download the complete sample solution zip [here](https:   ). Several files aren't included in the Github repository due to file size limits. The zip file has size ____. Fully unzipped it has size ___.

---

## 0. Download the Tutorial

Download ["Tutorial for Qwen2_VL_2b (IoT)" from Qualcomm Package Manager (QPM)](https://qpm.qualcomm.com/#/main/tools/details/Tutorial_for_Qwen2_VL_2b_IoT).
...
The `ar*-ar*-cl*` folder can be named anything within the pattern. This gives contestants more flexibilty. In this sample solution, the complete folder name is `ar128-ar1-cl2048` and only contains 1 file. 

The `embedding_weights*.raw` file can be named anything within the pattern. This gives contestants more flexibilty. In this sample solution, the complete folder name is `embedding_weights_151936x1536.raw`
...
This file contains the parameters needed to run model inference. In the tutorial, it was hard-coded within the notebooks for Qwen2-vl-2B. In order to support multiple models and not just Qwen2-vl-2B in our inference script, additional parameters need to be provided by the contestant.
...
Zip the folder, also naming the zip file with your team name. Your final submission should look like:
...
If you have a Snapdragon 8 Gen 5 Android device, here are the steps for running your models on the device. Place all your files within the `contestant_uploads` folder. It should not be your team_name folder or zip.
